...
class SinglyLinkedList {
  ...
  SinglyLinkedList(Node * n) {//pass by reference passing address of node
    head = n;
  }
  ...
  void appendNode(Node * n) {
      if (head == NULL) {// if linked list is empty
        ...
        Node * ptr = head;
        while (ptr -> next != NULL) {// traversing list
          ptr = ptr -> next;
        }
        ptr -> next = n;
      }
  ...
  void printList() {
    if (head == NULL) {
      ...
      while (temp != NULL) {
        cout << "Game " << temp -> key << " Score: " << temp -> data << " --> ";
        temp = temp -> next;
      }
    }
...
vector<int> gameFunction(char *wordinFunction, char *userWordinFunction, int letterNumber);   //Function for checking the letter, incrementing counters etc.
...
void printScore(int totalTrue,int totalFail, int count, SinglyLinkedList* s);
int main(void)
{	
	int count=1;
	...
	while(restartGame==1)
	{
		...
		vector<int> arr= gameFunction(askedWord, userWord, (gameType+ 5));    //Program goes to gameFunction with neccessary arrays and lettter size
		int totalTrue = arr[0];
		int totalFail = arr[1];
		
		printScore(totalTrue,totalFail,count,s);
		count++;
...
void printScore(int totalTrue,int totalFail, int count, SinglyLinkedList* s){
	Node* n1 = new Node();
	n1->key = count;
	n1->data = (totalTrue*100)/(totalFail+totalTrue);
	s->appendNode(n1);
	s->printList();
}
